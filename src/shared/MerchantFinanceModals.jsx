...
const blankPayment = { bankName: '', bankBranch: '', bankAccount: '', bankOwner: '', walletName: '', walletEmail: '', walletAccount: '', walletOwner: '', trc20: '', erc20: '', bep20: '' };

export default function MerchantFinanceModals({ client, merchant, action: requestedAction, onClose, actor = 'Admin', onChanged }) {
  const action = requestedAction === 'Payment' ? 'Payment Methods' : requestedAction === 'Withdrawals' ? 'Payment' : requestedAction;
  ...
  const loadPayments = async () => {
    ...
    (data || []).forEach(({ method_type: type, details = {} }) => {
      ...
      if (type === 'digital_currency') Object.assign(next, { trc20: details.trc20 || details.usdt_trc20 || '', erc20: details.erc20 || details.usdt_erc20 || '', bep20: details.bep20 || details.usdt_bep20 || '' });
    });
    setPayment(next);
  };
  ...
  const savePayment = async (event) => {
    ...
    const rows = [
      ...
      { seller_id: merchant.userId, method_type: 'digital_currency', details: { trc20: payment.trc20, erc20: payment.erc20, bep20: payment.bep20 }, updated_at: new Date().toISOString() },
    ];
  ...
  return <div className="merchant-finance-overlay" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    {action === 'Payment Methods' && <form className="merchant-finance-modal payment" onSubmit={savePayment}><header><div><h3>Payment — {merchant.email}</h3><p>View or update this seller's saved payout details.</p></div><button type="button" onClick={onClose}>×</button></header><h4>Bank Card</h4><div className="payment-grid"><label>Name<input value={payment.bankOwner} onChange={updatePayment('bankOwner')} placeholder="Account holder name" /></label><label>Bank Name<input value={payment.bankName} onChange={updatePayment('bankName')} placeholder="Bank name" /></label><label>Branch Name<input value={payment.bankBranch} onChange={updatePayment('bankBranch')} placeholder="Branch name" /></label><label>Account No.<input value={payment.bankAccount} onChange={updatePayment('bankAccount')} placeholder="Account or card number" /></label></div><h4>E-Wallet</h4><div className="payment-grid"><label>Name<input value={payment.walletOwner} onChange={updatePayment('walletOwner')} placeholder="Wallet owner name" /></label><label>Wallet Name<input value={payment.walletName} onChange={updatePayment('walletName')} placeholder="Wallet provider" /></label><label>Wallet Email<input type="email" value={payment.walletEmail} onChange={updatePayment('walletEmail')} placeholder="Wallet email" /></label><label>Account No.<input value={payment.walletAccount} onChange={updatePayment('walletAccount')} placeholder="Wallet account number" /></label></div><h4>Crypto Address</h4><label>USDT (TRC20)<input value={payment.trc20} onChange={updatePayment('trc20')} placeholder="TRC20 address" /></label><label>USDT (ERC20)<input value={payment.erc20} onChange={updatePayment('erc20')} placeholder="ERC20 address" /></label><label>USDT (BEP20)<input value={payment.bep20} onChange={updatePayment('bep20')} placeholder="BEP20 address" /></label>{message && <p className="merchant-modal-error">{message}</p>}<footer><button type="button" onClick={onClose}>Cancel</button><button type="submit" disabled={busy}>{busy ? 'Saving…' : 'Confirm'}</button></footer></form>}
