...
export default function MerchantActivityModals({ client, merchant, action, onClose, onChanged, openAction, actor = 'Admin' }) {
  ...
  const Header = ({ title }) => <><header><div className="activity-identity"><b>{merchant.name?.[0]?.toUpperCase()}</b><div><h3>{title}</h3><p>{merchant.email}</p></div></div><button type="button" onClick={onClose}>×</button></header>{action === 'Order' && <div className={`manual-buyer-mode mode-${orderBuyerId}`}><label>Buyer assignment<select disabled={busy} value={orderBuyerId} onChange={(event) => { const mode = event.target.value; setOrderBuyerId(mode); setPickingBuyers(false); if (mode !== 'selected') setOrderBuyerIds([]); if (mode !== 'random_country') setOrderBuyerCountry(''); }}><option value="random">Random buyers</option><option value="random_country">Random buyers from country</option><option value="selected">Selected buyers</option></select></label>{orderBuyerId === 'random_country' && <label>Buyer country<select required disabled={busy} value={orderBuyerCountry} onChange={(event) => setOrderBuyerCountry(event.target.value)}><option value="">— Select a country —</option>{buyerCountries.map(country => <option key={country} value={country}>{country}</option>)}</select></label>}{orderBuyerId === 'selected' && <button type="button" disabled={busy} onClick={() => setPickingBuyers(true)}>Select buyers {orderBuyerIds.length ? `(${orderBuyerIds.length})` : ''}</button>}<p>{orderBuyerId === 'random_country' ? 'Buyers are selected automatically from the chosen country.' : orderBuyerId === 'selected' ? 'Each product is ordered for every selected buyer.' : 'Buyers are selected automatically at random.'}</p></div>}</>;
