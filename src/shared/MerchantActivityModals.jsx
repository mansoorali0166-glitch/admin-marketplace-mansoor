...
import './MerchantOrderControls.css';
import { buildMerchantOrders } from './merchantOrderBatch';
import BuyerAssignmentModal from './BuyerAssignmentModal';
...
export default function MerchantActivityModals({ client, merchant, action, onClose, onChanged, openAction, actor = 'Admin' }) {
  ...
  const [buyerSetupOpen, setBuyerSetupOpen] = useState(action === 'Order');
  ...
  const load = async () => {
    ...
    const [orderRes, txnRes, lockRes, clickRes, profileRes, showcaseRes, buyersRes] = await Promise.all([
      ...
      client.from('showcase_products').select('on_shelf,products(id,name,product_code,image_url,sell_price,cost_price)').eq('seller_id', merchant.userId),
  ...
  useEffect(() => { load(); setTab(action === 'Order' ? 'Orders' : action === 'Manage' ? 'Balance' : 'Overview'); setShowOrderForm(action === 'Order'); setBuyerSetupOpen(action === 'Order'); setOrderHistoryOpen(false); }, [merchant?.userId, action]);
  ...
  const Header = ({ title }) => <><header><div className="activity-identity"><b>{merchant.name?.[0]?.toUpperCase()}</b><div><h3>{title}</h3><p>{merchant.email}</p></div></div><button type="button" onClick={onClose}>×</button></header>{action === 'Order' && <div className={`manual-buyer-mode mode-${orderBuyerId}`}><button type="button" disabled={busy} onClick={() => setBuyerSetupOpen(true)}>Choose Buyers</button><strong>{orderBuyerId === 'random_country' ? `Random from ${orderBuyerCountry || 'country'}` : orderBuyerId === 'selected' ? `${orderBuyerIds.length} selected buyer(s)` : 'Random buyers'}</strong></div>}{action === 'Order' && buyerSetupOpen && <BuyerAssignmentModal mode={orderBuyerId} country={orderBuyerCountry} countries={buyerCountries} buyers={buyers} buyerIds={orderBuyerIds} productCount={orderItems.length} busy={busy} onModeChange={(mode) => { setOrderBuyerId(mode); setPickingBuyers(false); if (mode !== 'selected') setOrderBuyerIds([]); if (mode !== 'random_country') setOrderBuyerCountry(''); }} onCountryChange={setOrderBuyerCountry} onBuyerIdsChange={setOrderBuyerIds} onAddProducts={() => { setBuyerSetupOpen(false); setPickingProducts(true); }} onClose={() => setBuyerSetupOpen(false)} />}</>;
  ...
  if (orderOnly) return <>
    {buyerSetupOpen && <BuyerAssignmentModal mode={orderBuyerId} country={orderBuyerCountry} countries={buyerCountries} buyers={buyers} buyerIds={orderBuyerIds} productCount={orderItems.length} busy={busy} message={message} onModeChange={(mode) => { setOrderBuyerId(mode); if (mode !== 'selected') setOrderBuyerIds([]); if (mode !== 'random_country') setOrderBuyerCountry(''); }} onCountryChange={setOrderBuyerCountry} onBuyerIdsChange={setOrderBuyerIds} onAddProducts={() => { setBuyerSetupOpen(false); setPickingProducts(true); }} onPlaceOrder={createOrder} onHistory={() => { setBuyerSetupOpen(false); setOrderHistoryOpen(true); }} onClose={onClose} />}
    {pickingProducts && <div className="merchant-order-picker-overlay"><section className="merchant-order-picker-modal"><header><h3>Select Products</h3><button type="button" onClick={() => { setPickingProducts(false); setBuyerSetupOpen(true); }}>×</button></header><div className="merchant-order-picker-table"><div className="merchant-order-picker-head product"><span></span><span>Image</span><span>Product Name</span><span>Unit Price (USD)</span></div>{showcaseProducts.map(product => <label className="merchant-order-picker-row product" key={product.id}><input type="checkbox" disabled={busy} checked={orderItems.some(item => item.productId === String(product.id))} onChange={() => toggleOrderProduct(String(product.id))} /><img src={product.image_url} alt="" /><span>{product.name || product.product_code}</span><strong>${Number(product.sell_price || 0).toFixed(2)}</strong></label>)}{!showcaseProducts.length && <p className="activity-empty">This seller has no on-shelf showcase products.</p>}</div><footer><span>{orderItems.length} product{orderItems.length === 1 ? '' : 's'} selected</span><button type="button" onClick={() => { setPickingProducts(false); setBuyerSetupOpen(true); }}>Confirm</button></footer></section></div>}
    {orderHistoryOpen && <div className="merchant-order-picker-overlay"><section className="merchant-order-picker-modal order-history-modal"><header><div><h3>Order History</h3><p>{merchant.email}</p></div><button type="button" onClick={() => { setOrderHistoryOpen(false); setBuyerSetupOpen(true); }}>×</button></header><div className="merchant-order-history-list">{orders.map(row => <article key={row.id}><div><b>{row.order_no || row.id}</b><span>{row.product_name || 'Product'}</span></div><strong>${(Number(row.sell_price || row.amount || 0) * Number(row.quantity || 1)).toFixed(2)}</strong><span>{row.status || '—'}</span><time>{new Date(row.created_at).toLocaleString()}</time></article>)}{!orders.length && <p className="activity-empty">No order history found.</p>}</div><footer><span>{orders.length} order{orders.length === 1 ? '' : 's'}</span><button type="button" onClick={() => { setOrderHistoryOpen(false); setBuyerSetupOpen(true); }}>Back</button></footer></section></div>}
  </>;
  ...
  return <div className={`merchant-activity-overlay ${managing ? 'manage-workspace-overlay' : ''} ${detailsDrawer ? 'merchant-details-drawer-overlay' : ''} ${orderOnly ? 'order-workspace-overlay' : ''}`}><section className={`merchant-activity-modal details ${managing ? 'manage-workspace' : ''} ${detailsDrawer ? 'merchant-details-drawer' : ''} ${orderOnly ? 'manual-order-workspace' : ''}`}>{managing ? <><button type="button" className="manage-back" onClick={onClose}>← Back</button><div className="manage-merchant-header"><div><h2>{merchant.name} {profile?.traffic_enabled === false && <small>CLICKS PAUSED</small>}</h2><p>{merchant.email} · {profile?.referral_code || merchant.id}</p></div><div className="manage-action-bar"><button onClick={() => openAction?.('Balance')}>＄ Adjust Balance</button><button onClick={() => openAction?.('Reset Pwd')}>⚿ Reset Password</button><button onClick={() => openAction?.('Lock Account')}>▣ Lock Account</button><button onClick={() => openAction?.('Login')}>↪ Login as Seller</button><button onClick={() => openAction?.(profile?.traffic_enabled === false ? 'Resume Clicks' : 'Stop Clicks')}>◉ {profile?.traffic_enabled === false ? 'Resume' : 'Stop'} Clicks</button><button onClick={() => openAction?.('Add Clicks')}>⌁ Add Clicks</button><button onClick={() => setTab('Click Logs')}>〽 Click Logs</button><button className="danger" onClick={() => openAction?.('Lock Shop')}>▣ Lock Shop</button></div></div></> : <Header title={orderOnly ? 'Manual Order' : merchant.name} />}{!orderOnly && <nav>{tabs.map(([name,total]) => <button type="button" className={tab === name ? 'active' : ''} onClick={() => setTab(name)} key={name}>{name} {!managing && name !== 'Overview' && name !== 'Info' ? `(${total})` : ''}</button>)}</nav>}
    ...
    {tab === 'Orders' && <div className={`merchant-orders-tab ${orderOnly ? 'order-only' : ''}`}>{!orderOnly && <div className="manage-table-tools order-tools"><input placeholder="Search orders..." value={orderSearch} onChange={(e)=>setOrderSearch(e.target.value)}/><select value={orderStatus} onChange={(e)=>setOrderStatus(e.target.value)}>{['all','Pending Payment','Pending Ship','Pending Receive','Shipped','Completed','Refund','Cancelled'].map(x=><option value={x} key={x}>{x === 'all' ? 'All statuses' : x}</option>)}</select><button type="button" className="new-merchant-order" onClick={() => setShowOrderForm(!showOrderForm)}>＋ Create Order</button></div>}{showOrderForm && <form className="merchant-order-form" onSubmit={createOrder}><div className="merchant-batch-orders"><header className="manual-order-header"><h3>Manual Order</h3><p>Create orders for this merchant using its live on-shelf products and virtual buyers.</p></header><div className="manual-order-shop"><span>Shop</span><strong>{merchant.email}</strong></div><div className="manual-order-actions"><div><span>Products</span><button type="button" disabled={busy} onClick={() => setPickingProducts(true)}>Select Products</button></div><div><span>Buyers</span><button type="button" disabled={busy} onClick={() => setPickingBuyers(true)}>Select Buyers {orderBuyerIds.length ? `(${orderBuyerIds.length})` : ''}</button></div></div><section className="manual-selected-products"><div className="manual-products-head"><span>Image</span><span>Product Name</span><span>Unit Price (USD)</span><span>Qty</span><span>Operation</span></div>{orderItems.map(item => { const product = showcaseProducts.find(product => String(product.id) === item.productId); return <div className="manual-product-row" key={item.productId}><img src={product?.image_url} alt="" /><strong>{product?.name || product?.product_code || 'Unavailable product'}</strong><span>${Number(product?.sell_price || 0).toFixed(2)}</span><input aria-label={`Quantity for ${product?.name || product?.product_code || 'product'}`} type="number" min="1" step="1" required disabled={busy} value={item.quantity} onChange={event => setOrderItems(current => current.map(row => row.productId === item.productId ? {...row, quantity: event.target.value} : row))} /><button type="button" disabled={busy} onClick={() => toggleOrderProduct(item.productId)}>Remove</button></div>; })}{!orderItems.length && <p className="activity-empty">No products selected.</p>}</section><div className="manual-order-summary"><strong>Total Amount (USD)</strong><b>${orderItems.reduce((total, item) => { const product = showcaseProducts.find(product => String(product.id) === item.productId); return total + Number(product?.sell_price || 0) * Number(item.quantity || 0); }, 0).toFixed(2)}</b></div><p className="manual-buyer-note">{orderBuyerIds.length ? `${orderBuyerIds.length} buyer${orderBuyerIds.length === 1 ? '' : 's'} selected. Each selected product will be ordered for every selected buyer.` : 'No buyer selected: a random virtual buyer will be assigned to each product.'}</p>{!buyers.length && <p className="activity-error">Create a virtual buyer before creating orders.</p>}<div className="manual-order-footer"><button type="submit" disabled={busy || !orderItems.length || !buyers.length}>{busy ? 'Creating…' : 'Place Order'}</button><button type="button" disabled={busy} onClick={() => { setOrderItems([]); setOrderBuyerIds([]); }}>Reset</button></div></div></form>}{(pickingProducts || pickingBuyers) && <div className="merchant-order-picker-overlay" onMouseDown={(event) => event.target === event.currentTarget && (pickingProducts ? setPickingProducts(false) : setPickingBuyers(false))}><section className="merchant-order-picker-modal"><header><h3>{pickingProducts ? 'Select Products' : 'All Buyers'}</h3><button type="button" onClick={() => { setPickingProducts(false); setPickingBuyers(false); }}>×</button></header><div className="merchant-order-picker-table">{pickingProducts ? <><div className="merchant-order-picker-head product"><span></span><span>Image</span><span>Product Name</span><span>Unit Price (USD)</span></div>{showcaseProducts.map(product => <label className="merchant-order-picker-row product" key={product.id}><input type="checkbox" disabled={busy} checked={orderItems.some(item => item.productId === String(product.id))} onChange={() => toggleOrderProduct(String(product.id))} /><img src={product.image_url} alt="" /><span>{product.name || product.product_code}</span><strong>${Number(product.sell_price || 0).toFixed(2)}</strong></label>)}{!showcaseProducts.length && <p className="activity-empty">This seller has no on-shelf showcase products.</p>}</> : <><div className="merchant-order-picker-head buyer"><span></span><span>Name</span><span>Phone</span><span>Address</span></div>{buyers.map(buyer => <label className="merchant-order-picker-row buyer" key={buyer.id}><input type="checkbox" disabled={busy} checked={orderBuyerIds.includes(String(buyer.id))} onChange={() => toggleOrderBuyer(String(buyer.id))} /><strong>{buyer.name}</strong><span>{buyer.phone || '—'}</span><span>{[buyer.address, buyer.city, buyer.state, buyer.postal, buyer.country].filter(Boolean).join(', ') || '—'}</span></label>)}{!buyers.length && <p className="activity-empty">No virtual buyers are available.</p>}</>}</div><footer><span>{pickingProducts ? `${orderItems.length} product${orderItems.length === 1 ? '' : 's'} selected` : `${orderBuyerIds.length} buyer${orderBuyerIds.length === 1 ? '' : 's'} selected`}</span><button type="button" onClick={() => { setPickingProducts(false); setPickingBuyers(false); }}>Confirm</button></footer></section></div>}{message && <p className="activity-message">{message}</p>}<div className="manage-table-scroll"><div className={`order-management-table ${orderOnly ? 'order-only-table' : ''}`}><div className="order-management-row head"><span>ORDER</span><span>PRODUCT</span><span>AMOUNT</span><span>STATUS</span><span>SHIPPING</span><span>DATE</span>{(!orderOnly || agentOrderControls) && <span>ACTIONS</span>}</div>{visibleOrders.map(row=><div className="order-management-row" key={row.id}><b>{row.order_no||row.id}</b><span>{row.product_name||'—'}</span><strong>${(Number(row.sell_price||0)*Number(row.quantity||1)).toFixed(2)}</strong><span>{row.status||'—'}</span><span>{row.shipping_address||'—'}</span><time>{new Date(row.created_at).toLocaleString()}</time>{!orderOnly && <select value={row.status} disabled={busy} onChange={(e)=>updateOrderStatus(row.id,e.target.value)}>{['Pending Payment','Pending Ship','Completed','Refund','Cancelled','Rejected'].map(x=><option key={x}>{x}</option>)}</select>}{agentOrderControls && (String(row.status).toLowerCase() === 'pending payment' || String(row.status).toLowerCase() === 'pending pay' ? <span>Waiting for seller payment</span> : <select value={row.status} disabled={busy} onChange={(e)=>updateOrderStatus(row.id,e.target.value)}>{['Pending Ship','Completed','Refund','Cancelled','Rejected'].map(x=><option key={x}>{x}</option>)}</select>)}</div>)}{!visibleOrders.length&&<p className="activity-empty">No orders found.</p>}</div></div></div>}
