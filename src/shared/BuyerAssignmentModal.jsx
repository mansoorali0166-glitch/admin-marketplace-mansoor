...
export default function BuyerAssignmentModal({ mode, country, countries, buyers, buyerIds, productCount, busy, onModeChange, onCountryChange, onBuyerIdsChange, onAddProducts, onClose }) {
  ...
  return <div className="buyer-assignment-overlay" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="buyer-assignment-modal" role="dialog" aria-modal="true" aria-labelledby="buyer-assignment-title">
      ...
      <footer><span>{mode === 'selected' ? `${buyerIds.length} buyer(s) selected` : 'Automatic buyer assignment'}</span><button type="button" onClick={onClose}>Done</button></footer>
    </section>
  </div>;
}
